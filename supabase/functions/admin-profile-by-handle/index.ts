...
serve(async (req) => {
  ...
  try {
    ...
    // Resolve by username/short_id/id/public_id(user_<n>)
    ...
    const profile = await admin
      .from("profiles")
      .select("id, subscription_status, is_premium, subscription_expires_at, premium_since, plan_type, payment_provider, payment_id, payment_status, provider_payment_id, provider_subscription_id, updated_at")
      .eq("id", userId)
      .maybeSingle();
    return new Response(JSON.stringify({ ok: true, user_id: userId, profile: profile.data || null }), {
