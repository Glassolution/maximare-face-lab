...
-- 2. Final Secure RPC Function with Telemetry
...
DECLARE
  ...
  -- Rate Limit Config
  max_checks int := 120; -- Increased tolerance
...
BEGIN
  ...
  -- 2. Verify Ownership & Existence (Lock Row)
  ...
  IF payment_record IS NOT NULL AND payment_record.user_id != current_user_id THEN
    RETURN json_build_object('success', false, 'error', 'Forbidden: Payment does not belong to user.');
  END IF;

  -- B. RATE LIMITING (Cache Strategy)
  ...
  -- 2. Telemetry: Set approved_at if newly approved
  IF pay_status = 'approved' AND (payment_record IS NULL OR payment_record.status != 'approved') THEN
      UPDATE payments SET approved_at = NOW() WHERE payment_id = payment_id_input;
  END IF;
